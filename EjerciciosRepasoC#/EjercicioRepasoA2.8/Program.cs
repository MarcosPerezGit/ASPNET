...
//using System.Linq; // LINQ removido
using System.Text;
...
class Circulo : Figura
{
    ...
    public double Radio { get => _radio; set => _radio = value <= 0 ? 1 : value; } /* TAMBIEN SE PUEDE HACER SIN LAMDA: 
                                                                                    * set
                                                                                        {
                                                                                            if (value > 0)
                                                                                                _radio = value;
...
class Programa
{
    ...
    static void CrearFigura()
    {
        ...
        else if (figura.Equals("rectangulo"))
        {
            ...
            double h;
            if (double.TryParse(Console.ReadLine(), out b) && double.TryParse(Console.ReadLine(), out h))
            {
        ...
        else if (figura.Equals("rombo"))
        {
            ...
            double menor;
            if (double.TryParse(Console.ReadLine(), out mayor) && double.TryParse(Console.ReadLine(), out menor))
            {
                Rombo rombo = new Rombo();
                rombo.DiagonalMayor = mayor;
                rombo.DiagonalMenor = menor;
                figuras.Add(rombo);
            }
            else
            {
        ...
        {
            Console.WriteLine("Opción de figura no válida.");
        }
    }
    //  Requisito funcional      
    ...
    static void CalcularAreaTotal()
    {
        ...
        foreach (var f in figuras)
            suma += f.CalcularArea();
        Console.WriteLine($"Area total = {suma}");
    }
    //  Requisito funcional      
    static void CalcularPerimetroTotal()
    {
        ...
        Console.WriteLine($"Perímetro total = {suma}");
    }
